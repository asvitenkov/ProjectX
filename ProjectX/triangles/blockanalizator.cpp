...
BlockAnalizator::BlockAnalizator(QVector<TriangleShared> & , QVector<TPoint2> *pt, QObject *parent) :
    QThread(parent),  triangles( ), points(pt)
{
}
...
bool BlockAnalizator::crossedRadius(TriangleShared t1, TriangleShared t2)//пересекаются ли радиусы треугольников?
{
    double d;
...
bool BlockAnalizator::sameTriangle(TriangleShared t1, TriangleShared t2)//является ли соседним треугольник?
{
            ...
            for(int j = i+1; j <  triangles.size(); ++j)
            {
                if((! triangles[j].dead) && (!sameTriangle( triangles[i], triangles[j])))
                {
                    if(crossedRadius( triangles[i], triangles[j]))
                        if( triangles[i].crossedTriangles( triangles[j]))
                        {
